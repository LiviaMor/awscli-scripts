#!/bin/bash
# Este script assume uma role temporária e lança instâncias EC2 usando as credenciais temporárias.
...
# Perguntar nome da instância (tag Name)
read -p "Digite um nome para a instância (tag Name) [sem-nome]: " INSTANCE_NAME
INSTANCE_NAME=${INSTANCE_NAME:-sem-nome}
...
aws ec2 run-instances \
  ...
  --region us-east-1 \
  --count 2 \
  --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$INSTANCE_NAME},{Key=grupo,Value=AutomacaoAWSCLI}]" 